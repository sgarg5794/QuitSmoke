...
public class DaywiseDataVisualizationActivity extends AppCompatActivity
implements  OnChartValueSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_daywise_data_visualization);
        ...
        List<CigCountDayPeriod> dvLocation = db.getCigConsumedTodayDayPer();
        ...
        bottomBar.setOnTabSelectListener(new OnTabSelectListener() {
            ...
            public void onTabSelected(@IdRes int tabId) {
                if(tabId==R.id.tab_recents)
                    setData(db.getCigConsumedTodayDayPer());
                else if(tabId==R.id.tab_favorites)
                    setData(db.getCigConsumedWeekDayPer());
                else if(tabId==R.id.tab_nearby)
                    setData(db.getCigConsumedMonthDayPer());
                else if(tabId==R.id.tab_friends)
                    setData(db.getCigConsumedYearDayPer());
    ...
    private void setData(List<CigCountDayPeriod> locationSmoke) {
        float mult = 0f;
        for (int i = 0; i < locationSmoke.size(); i++) {
            Log.i("Number of Cig", String.valueOf(locationSmoke.get(i).getCigCountPer()));
            mult = mult + locationSmoke.get(i).getCigCountPer();
        }
        ...
        for (int i = 0; i < locationSmoke.size(); i++) {
            ...
            entries.add(new PieEntry((float) (locationSmoke.get(i).getCigCountPer()), locationSmoke.get(i).getPeriodName()));
    ...
    private SpannableString generateCenterSpannableText() {
        ...
        SpannableString s = new SpannableString("DayPeriodWiseSmoking\ndeveloped by Group 8");
    ...
    public void onValueSelected(Entry e, Highlight h) {
        ...
        Toast.makeText(getApplicationContext(),String.valueOf((int) e.getY()) + " Cigarette(s) during " + pEntry.getLabel() , Toast.LENGTH_SHORT).show();
    }
